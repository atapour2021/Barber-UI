...
@Component({
  selector: 'app-home',
  standalone: true,
  imports: [RouterLink, IonHeader, IonToolbar, IonTitle, IonContent, IonSpinner, IonCard, IonCardHeader, IonCardTitle, IonCardContent, IonButton, IonSegment, IonSegmentButton, IonIcon, EmptyStateComponent],
  template: `
  ...
  <ion-content>
    <div class="page-wrap">
      ...
      <ion-segment [value]="tab" (ionChange)="tab=($event.detail.value ?? 'shops')!.toString()" style="margin-top:14px">
      ...
      @if (tab === 'barbers') {
        ...
        <ion-button expand="block" routerLink="/barbers" style="margin-top:12px">{{t.manageBarbers}}</ion-button>
      }
      ...
      @if (tab === 'services') {
        ...
        <ion-button expand="block" routerLink="/services" style="margin-top:12px">{{t.manageServices}}</ion-button>
      }

      <ion-button expand="block" color="secondary" routerLink="/appointments" style="margin-top:16px"><ion-icon name="calendar-outline" slot="start"></ion-icon> {{t.bookAppointment}}</ion-button>
    </div>
  </ion-content>`,
})
