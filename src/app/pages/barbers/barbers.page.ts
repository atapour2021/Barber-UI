        ...
        <ion-list lines="none" style="background:transparent;width:100%">
          @for (b of items(); track b.id) {
            <ion-card button [routerLink]="['/barbers', b.id]">
              ...
              >
                <ion-label
                  ><h3 style="font-weight:700">{{ b.fullName }}</h3>
                  <p class="muted">{{ b.bio ?? '' }}</p>
                  <p>
                    <ion-badge [color]="b.isAvailable ? 'success' : 'medium'">{{
                      b.isAvailable ? t.available : t.busy
                    }}</ion-badge>
          ...
          <ng-template>
            ...
            <ion-content class="ion-padding" [fullscreen]="true">
              <div class="page-wrap">
                ...
                />
                <app-ui-input
                  [label]="t.barbershopId"
                  [(ngModel)]="form.barbershopId"
                />
                <app-ui-select
                  [label]="t.status"
                  [(ngModel)]="form.status"
                ...
                />
                <ion-item
                  ><ion-toggle [(ngModel)]="form.isAvailable">{{
                    t.available
                  }}</ion-toggle></ion-item
                >
                <app-ui-button icon="create-outline" (pressed)="save()">{{
...
export class BarbersPage implements OnInit {
  ...
  c = fa.common;
  statusOpts = [
    { value: 'active', label: fa.barbers.statusActive },
    { value: 'inactive', label: fa.barbers.statusInactive },
  ];
  items = signal<Barber[]>([]);
  ...
  open() {
    this.editId = '';
    this.form = { status: 'active', isAvailable: true };
    this.show.set(true);
  }
  save() {
    const obs = this.editId
      ? this.api.barbers.update(this.editId, this.form)
      : this.api.barbers.create(this.form);
    obs.subscribe({
      next: () => {
        this.show.set(false);
        this.load();
