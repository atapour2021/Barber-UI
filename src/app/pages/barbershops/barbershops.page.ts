  ...
  template: `
    <ion-content [fullscreen]="true">
      <div class="page-wrap" dir="rtl">
        <div class="section-head"><h3>{{ t.title }}</h3><span class="muted">{{ shops().length }} مورد</span></div>
        @if (loading()) {
          ...
          @for (b of shops(); track b.id) {
            <ion-card style="margin-bottom:10px">
              <ion-card-content style="display:flex;justify-content:space-between;align-items:center;gap:10px">
                <ion-label style="min-width:0"><h3 style="font-weight:800;color:var(--text-primary);font-size:13px">{{ b.name }}</h3><p class="muted" style="margin:2px 0 0">{{ b.address }}</p></ion-label>
                @if (isAdmin) {
                  <ion-button fill="clear" size="small" color="danger" (click)="remove(b.id)"><ion-icon name="trash-outline" slot="icon-only"></ion-icon></ion-button>
                }
              </ion-card-content>
            </ion-card>
          }
          ...
          <ion-modal [isOpen]="showForm()" (didDismiss)="showForm.set(false)">
            <ng-template>
              <ion-content class="ion-padding" [fullscreen]="true">
                <div class="page-wrap" dir="rtl">
                  <div class="section-head"><h3>{{ t.newShop }}</h3><ion-button fill="clear" size="small" (click)="showForm.set(false)">{{ c.close }}</ion-button></div>
...
export class BarbershopsPage implements OnInit {
  ...
  ngOnInit() { this.load(); }
  load() { this.loading.set(true); this.api.barbershops.list().subscribe({ next: (v) => { this.shops.set(v as Barbershop[]); this.loading.set(false); }, error: () => this.loading.set(false) }); }
  create() { this.api.barbershops.create(this.form).subscribe({ next: () => { this.showForm.set(false); this.load(); } }); }
